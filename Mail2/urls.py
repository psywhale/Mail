...
from .views import IndexView,ReplyView, \
    ...
    AuditViewUser, Launch, FileUpload, DownloadView, GetEmailListView, UnarchiveMailView

urlpatterns= [
    url(r'^$',IndexView.as_view(), name='IndexView'),
    url(r'^reply/(?P<id>[0-9]+)/$', ReplyView.as_view(), name='reply'),
    url(r'^reply/(?P<id>[0-9]+)/(?P<userid>[0-9]+)/(?P<sn>[0-9Cc]{5}\-[0-9]{3}[sS]+)/$', ReplyView.as_view(), name='reply'),
    ...
    url(r'^unarchive/$', UnarchiveMailView.as_view(), name='unarchivemail'),
